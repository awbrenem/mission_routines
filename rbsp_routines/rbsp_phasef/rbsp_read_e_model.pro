...
;---Init settings.
    type_dispatch = hash()
    valid_range = (probe eq 'a')? time_double(['2012-09-08','2019-10-15']): $
        time_double(['2012-09-08','2019-07-17'])
    rbspx = 'rbsp'+probe
    base_name = rbspx+'_e_model_%Y_%m%d_'+version+'.cdf'
    local_path = [local_root,rbspx,'e_model','%Y']
    ; E model only.
...
probe = 'b'
rbsp_read_e_model, probe=probe, rbsp_info('efw_phasef', probe=probe)
stop
...
rbsp_efw_phasef_prepare_residue_removal, time_range, probe=probe
