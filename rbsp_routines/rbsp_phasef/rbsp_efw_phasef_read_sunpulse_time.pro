...
pro rbsp_efw_phasef_read_sunpulse_time, time, probe=probe, filename=file
...
;---Check inputs.
    ...
    if n_elements(datatype) eq 0 then datatype = 'sunpulse_time'
...
;---Init settings.
    ...
    local_path = [local_root,rbspx,'sunpulse_time','%Y']
    remote_path = [remote_root,'sunpulse_time',rbspx,'%Y']

    type_dispatch['sunpulse_time'] = dictionary($
        'pattern', dictionary($
