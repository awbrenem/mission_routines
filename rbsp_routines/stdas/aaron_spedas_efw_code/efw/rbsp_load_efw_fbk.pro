...
; $LastChangedDate: 2018-12-17 16:34:43 -0600 (Mon, 17 Dec 2018) $
...
for s=0,n_elements(p_var)-1 do begin
  ...
  ;If files have been loaded then continue
   if is_string(tns) then begin
     ...
     ;Create final FBK7 tplot variables
     if total(fbk7data.y[*,*,0]) ne 0. then store_data,fbknewname+'_fb1_av',data={x: fbk7data.x, $
        y:reform(fbk7data.y[*,*,0]), v: v7},dlimits={spec: 1, data_att: default_data_att}
     if total(fbk7data.y[*,*,1]) ne 0. then store_data,fbknewname+'_fb1_pk',data={x: fbk7data.x, $
        y:reform(fbk7data.y[*,*,1]), v: v7},dlimits={spec: 1, data_att: default_data_att}
     if total(fbk7data.y[*,*,2]) ne 0. then store_data,fbknewname+'_fb2_av',data={x: fbk7data.x, $
        y:reform(fbk7data.y[*,*,2]), v: v7},dlimits={spec: 1, data_att: default_data_att}
     if total(fbk7data.y[*,*,3]) ne 0. then store_data,fbknewname+'_fb2_pk',data={x: fbk7data.x, $
   ...
   ;...and load them
    ...
    if is_string(tns) then begin
     ...
     ;Create final FBK13 tplot variables
     get_data,fbkname,data=fbk13data
     if total(fbk13data.y[*,*,0]) ne 0. then store_data,fbknewname+'_fb1_av',data={x: fbk13data.x, $
        y:reform(fbk13data.y[*,*,0]), v: v13},dlimits={spec: 1, data_att: default_data_att}
     if total(fbk13data.y[*,*,1]) ne 0. then store_data,fbknewname+'_fb1_pk',data={x: fbk13data.x, $
        y:reform(fbk13data.y[*,*,1]), v: v13},dlimits={spec: 1, data_att: default_data_att}
     if total(fbk13data.y[*,*,2]) ne 0. then store_data,fbknewname+'_fb2_av',data={x: fbk13data.x, $
        y:reform(fbk13data.y[*,*,2]), v: v13},dlimits={spec: 1, data_att: default_data_att}
     if total(fbk13data.y[*,*,3]) ne 0. then store_data,fbknewname+'_fb2_pk',data={x: fbk13data.x, $
