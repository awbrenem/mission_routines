...
; $LastChangedDate: 2017-04-05 14:36:48 -0500 (Wed, 05 Apr 2017) $
...
skeletonFile = join_path([srootdir(),skeleton])
...
; Sheng: this code doesn't run, use flag_all to fill in the eclipse flag.
;;*****TEMPORARY CODE*****
...
; Sheng: need to fill in L_vector.
...
; Sheng: boom_shorting_factor and boom_length are missing, need to fill.
...
probes = ['a','b']
version = 'v01'
root_dir = '/Volumes/UserA/user_volumes/kersten/data/rbsp'
if file_test(root_dir) eq 0 then stop
...
foreach probe, probes do begin
    ...
    foreach day, days do begin
        date = time_string(day,tformat='YYYY-MM-DD')
        year = strmid(date,0,4)
        
        folder = root_dir+'/'+rbspx+'/l2/e-hires-uvw/'+year
        file = folder+'/'+rbspx+'_efw-l2_e-hires-uvw_'+time_string(day,tformat='YYYYMMDD')+'_'+version+'.cdf'
        if file_test(file) eq 1 then continue

        rbsp_efw_make_l2_esvy_uvw, probe, date, folder=folder
    endforeach
endforeach
