...
;   $LastChangedDate: 2016-08-16 08:35:57 -0500 (Tue, 16 Aug 2016) $
...
;path = '~/Desktop/'
;fnb = 'rbspb_mscb1_20170509_235830-235900_mgse.cdf'
;fne = 'rbspb_eb1_20170509_235830-235900_mgse.cdf'

pro rbsp_efw_burst_fa_rotate_crib_loop,fne,fnb,path
...
minduration = 0.2                ;minimum duration of each burst (sec). This is used to eliminate
...
;  for f=0,n_elements(fnb)-1 do begin


  cdf2tplot,path+fnb

  get_data,rbspx+'_efw_mscb'+bt+'_mgse',tt,bb
  t0 = tt[0]
  t1 = tt[n_elements(tt)-1]
  dt = time_double(t1) - time_double(t0)


  cdf2tplot,path+fne

  get_data,rbspx+'_efw_eb'+bt+'_mgse',tt,ee
;  t0 = tt[0]
;  t1 = tt[n_elements(tt)-1]
;  dt = time_double(t1) - time_double(t0)
...
;  if tdexists(rbspx+'_efw_vb'+bt+'_mgse',tr[0],tr[1]) then begin
;  if etype = 'v' then begin
...
;    get_data,rbspx+'_efw_vb'+bt+'_mgse',data=dd
...
if start_after_step lt 4 then begin
  ...
  if is_struct(varr) then begin
    ...
    for i=0,nchunks-1 do begin
      ...
      twavpol,varM_s4+'_tmp',prefix='tmp',nopfft=32768/2.,steplength=32768/4.
      ...
      twavpol,varE_s4+'_tmp',prefix='tmp',nopfft=32768/2.,steplength=32768/4.
