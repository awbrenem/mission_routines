...
;   $LastChangedDate: 2014-10-17 15:38:53 -0500 (Fri, 17 Oct 2014) $
...
;; Store pflux variables when finished. 


;;Something seems to happen to the electric field when you move into
;;the Chaston part of the crib. the ebb_mgse_bp_fa all have values <1,
;;whereas the mgse quantities are all in the 100s. See attached.

;;It would also be good to go through the tplot variables and then just save the ones that are useful. We do need to have the 3 components in mgse and in fac for E&Sc in tplot variables (which current version doesn't). 
...
if start_after_step le 2 then stop
...
;^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
if start_after_step le 3 then stop
...
;Ew and Bw in Pflux coord
   ...
   for i=0,nchunks-1 do begin
      ...
      if keyword_set(pflux_spec) then begin
         ...
         fmin2 = fmin
         fmax2 = fmax
         rbsp_poynting_spec,varE+'_tmp',varM+'_tmp',rbspx+'_Mag_mgse_tmp',df,fmin2,fmax2,min_sf=1000.,/noplot
         tplot,['ppara_p','ppara_n','ppara_b',rbspx+'_state_mlat_interp']
      endif else begin
         rbsp_poynting_flux,$
            varM+'_tmp',$
            varE+'_tmp',$
...
;^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
if start_after_step le 4 then stop
...
if start_after_step le 5 then stop
...
;	store_data,varE+'_samplerate_diff',data={x:varr.x,y:abs(sr-medavg[0])}
   ...
   store_data,varM+'_helict',data={x:chastontimesB[1:nn],y:helicitB[1:nn,*],v:freqvalsB}
   ...
   store_data,varE+'_helict',data={x:chastontimesE[1:nn],y:helicitE[1:nn,*],v:freqvalsE}
   ...
   get_data,varM+'_helict',data=tmp
   if goo[0] ne -1 then tmp.y[goo] = !values.f_nan
   store_data,varM+'_helict',data=tmp
   ...
   get_data,varE+'_helict',data=tmp
   if goo[0] ne -1 then tmp.y[goo] = !values.f_nan
   store_data,varE+'_helict',data=tmp
