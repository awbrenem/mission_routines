...
;***temp patch
;if n_elements(frel) gt n_m then frel = frel[0:n_m-1]
;if n_m gt n_elements(frel) then n_m = n_elements(frel)
...
window = hanning_stretch(n_elements(d2x),n_elements(d2x)/12L-2L)
