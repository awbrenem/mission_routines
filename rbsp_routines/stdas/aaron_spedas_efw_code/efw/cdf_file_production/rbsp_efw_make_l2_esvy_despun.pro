...
;     skeleton='/Volumes/UserA/user_homes/kersten/RBSP_l2/'+rbspx+'_efw-l2_00000000_v02.cdf'
     skeleton=homedir+'Code/tdas_svn_daily/general/missions/rbsp/efw/cdf_file_production/'+rbx+'efw-lX_00000000_vXX.cdf'
     ...
     if keyword_set(testing) then $
        skeletonfile = homedir+'Desktop/code/Aaron/RBSP/TDAS_trunk_svn/idl/general/missions/rbsp/efw/cdf_file_production/rbsp'+$
  ...
  if keyword_set(testing) then folder = homedir+'Desktop/code/Aaron/RBSP/TDAS_trunk_svn/idl/general/missions/rbsp/efw/cdf_file_production/'
...
;------------ Set up paths. END. ----------------------------
  ...
  ;Full cadence (32 Samples/sec) times 
  ...
  ;Once per second times that we'll use for the flag values
;  get_data,rbx+'state_mlt',data=tmp
;  times = tmp.x
;  epoch_v = tplot_time_to_epoch(times,/epoch16)


  ;Once per minute times we'll use for the ephemeris and flags data
  tr = timerange()
  times_ephem = 60.*dindgen(1440.) + tr[0]
  epoch = tplot_time_to_epoch(times_ephem,/epoch16)
...
;******************
  ...
  ;Downsample the GSE position and velocity variables to once/sec cadence
  varstmp = [rbx+'state_pos_gse',$
            rbx+'state_vel_gse',$
            rbx+'state_mlt',rbx+'state_mlat',rbx+'state_lshell',$
            rbx+'spinaxis_direction_gse']

  for qq=0,n_elements(varstmp)-1 do tinterpol_mxn,varstmp[qq],times_ephem,newname=varstmp[qq],/spline,/overwrite
  ...
  varsave_general = ['epoch','epoch_e','epoch_v','global_flag',$
                    'efield_mgse',$
