...
; $LastChangedDate: 2020-09-11 15:31:38 -0500 (Fri, 11 Sep 2020) $
...
;                                       year + path_sep()
 ...
 ; if ~keyword_set(no_cdf) then file_mkdir, folder
  ...
  if ~keyword_set(testing) then begin
    path = homedir+'Code/tdas_svn_daily/general/missions/rbsp/efw/cdf_file_production/'
    skeleton = path + skfile
  endif else begin
    path = homedir+'Desktop/code/Aaron/RBSP/TDAS_trunk_svn/idl/general/missions/rbsp/efw/cdf_file_production/'
    skeleton = path + skfile
  endelse
