...
; $LastChangedDate: 2020-04-29 13:19:47 -0500 (Wed, 29 Apr 2020) $
...
; Parameters from gain curve fit. Aaron's note: I think this is JBT's 
;fits to the MSC channel gain curve (inverted) I made for Wygant13 paper
;that comes from the CIT testing. 
...
if nind gt 0 then begin
  ...
  !p.multi = [0,2,3]
  plot,tmp_f,tmp_resp2,xrange=[0.1,1d6],/xlog,xtitle='freq(Hz)',ytitle='Gain Lowpass and highpass filters'
  plot,tmp_f,imaginary(tmp_resp2)/!dtor,xrange=[0.1,1d6],/xlog,xtitle='freq(Hz)',ytitle='Phase(deg) Lowpass and highpass filters'
  plot,tmp_f,ntv,xrange=[1,20000],/xlog,xtitle='freq(Hz)',ytitle='nT/V conversion',/xstyle
  plot,[0,0],[0,0]
  plot,tmp_f,tmp_resp,xrange=[0.1,1d6],/xlog,xtitle='freq(Hz)',ytitle='Gain, overall response'
  plot,tmp_f,imaginary(tmp_resp)/!dtor,xrange=[0.1,1d6],/xlog,xtitle='freq(Hz)',ytitle='Phase(deg), overall response'
stop
