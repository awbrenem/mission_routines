...
;   2021-01-14: AWB - removed despinning (UVW->DSC) capability. Wasn't working properly, and 
;           this can be done with rbsp_uvw_to_dsc.pro
;
;
; VERSION:
; $LastChangedBy: aaronbreneman $
; $LastChangedDate: 2019-01-29 12:01:48 -0600 (Tue, 29 Jan 2019) $
...
;boom_length2 = cp.boom_length
bl = rbsp_efw_boom_deploy_history(data.x)

;Change boom_length into an [n,3] array for appropriate sc
if probe eq 'a' then boom_length = [[bl.A12],[bl.A34],[bl.A56]]
if probe eq 'b' then boom_length = [[bl.B12],[bl.B34],[bl.B56]]


;Note: values of 1 are used here. The shorting factor is applied during later calibrations. 
