...
; $LastChangedDate: 2017-06-15 13:18:24 -0500 (Thu, 15 Jun 2017) $
...
pro rbsp_corotation_efield,probe,date,$
...
;Load SPICE if required          
  if ~tdexists('rbsp'+probe+'_spinaxis_direction_gse',tr[0],tr[1]) then rbsp_load_spice_cdf_file,probe
...
;***** I CAN CHANGE THIS SO THAT SHENG'S CDF FILES ARE CALLED **************
;*********************************************************
;  if ~keyword_set(data_preloaded) then rbsp_load_state,probe=probe,$
;      datatype=['pos','vel','spinper','spinphase','mat_dsc','Lvec'],_extra=extra

;  get_data,'rbsp'+probe+'_Lvec',data=lvec

;Load spice for position and velocity data
;  if ~keyword_set(data_preloaded) then $
;    rbsp_load_spice_state,probe=probe,coord='gse',_extra=extra


;;Get spin-axis pointing direction once/minute
;  if ~keyword_set(data_preloaded) then begin 
...
;
;  endif else begin
;      get_data,'rbsp'+probe+'_wsc_gse',data=wsc_GSE

  time4 = dindgen(1440.)*60. + time_double(date)

  tinterpol_mxn,'rbsp'+probe+'_spinaxis_direction_gse',time4
  get_data,'rbsp'+probe+'_spinaxis_direction_gse_interp',data=wsc_GSE
...
;  endelse
  ...
  ;  if ~keyword_set(data_preloaded) and level eq 'ql' then rbsp_load_emfisis,probe=probe,/quicklook
    if level eq 'l3' then rbsp_load_emfisis,probe=probe,coord='gse',cadence='1sec',level='l3'
    if level eq 'l2' then rbsp_load_emfisis,probe=probe,coord='uvw',level='l2'
    if level eq 'ql' then rbsp_load_emfisis,probe=probe,/quicklook
    ...
    if level eq 'ql' then begin
      ...
      rbsp_uvw_to_mgse,probe,'rbsp'+probe+'_emfisis_quicklook_Mag_tmp',_extra=extra


      get_data,'rbsp'+probe+'_emfisis_quicklook_Mag_tmp_mgse',data=Bmag
      wsc_gsetmp = [[interpol(wsc_GSE.y[*,0],time_double(time4),Bmag.x)],$
                    [interpol(wsc_GSE.y[*,1],time_double(time4),Bmag.x)],$
                    [interpol(wsc_GSE.y[*,2],time_double(time4),Bmag.x)]]

      rbsp_mgse2gse,'rbsp'+probe+'_emfisis_quicklook_Mag_tmp_mgse',wsc_gsetmp,probe=probe,$
                    newname='rbsp'+probe+'_emfisis_quicklook_Mag_gse',_extra=extra
    ...
    if level eq 'l2' then begin
      ...
      rbsp_uvw_to_mgse,probe,'rbsp'+probe+'_emfisis_l2_uvw_Mag_tmp',_extra=extra
  ...
  ;     time4 = time_string(time3) ;put into yyyy-mm-dd/hh:mm:ss format
      ...
      rbsp_mgse2gse,'rbsp'+probe+'_emfisis_l2_uvw_Mag_tmp_mgse',wsc_gsetmp,probe=probe,$
                    newname='rbsp'+probe+'_emfisis_l2_uvw_Mag_gse',_extra=extra
  ...
  endif ;data_preloaded


  if level eq 'ql' then copy_data,'rbsp'+probe+'_emfisis_quicklook_Mag_gse','bfield_data'
  if level eq 'l2' then copy_data,'rbsp'+probe+'_emfisis_l2_uvw_Mag_gse','bfield_data'
...
;Find corotation electric field
  ...
  gei_pos       = gei_pos.y
  xgei = gei_pos[*,0] & ygei = gei_pos[*,1] & zgei = gei_pos[*,2]
  ...
  wsc_GSE2 = [[interpol(wsc_GSE.y[*,0],time_double(time4),Ecoro.x)],$
              [interpol(wsc_GSE.y[*,1],time_double(time4),Ecoro.x)],$
              [interpol(wsc_GSE.y[*,2],time_double(time4),Ecoro.x)]]
