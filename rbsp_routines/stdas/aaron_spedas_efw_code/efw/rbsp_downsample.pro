...
pro rbsp_downsample,tnames,sr,nochange=nochange,suffix=suffix,win=win,ptswin=ptswin
	...
	;		endif
			...
			t0 = min(dat.x) & t1 = max(dat.x)
			newt = dindgen((t1-t0)*sr)/sr + t0

			;Test to see if variable is a [n] or [n,x] element array
			tst = size(dat.y,/n_dimensions) & tst2 = size(dat.y,/dimensions)
			...
			if n_pad_factor ne 0 then zero_arr = replicate(0,n_pad_factor) else zero_arr = -1

			;------------------------------------------------------
			if tst eq 1 then begin

				;zero pad
				if zero_arr[0] ne -1 then dat_tmp = [dat.y,zero_arr] else dat_tmp = dat.y
			...
			if tst gt 1 then begin
				...
				for b=0,tst2[1]-1 do begin
					...
					;zero pad
					if zero_arr[0] ne -1 then dat_tmp = [dat.y[*,b],zero_arr] else dat_tmp = dat.y[*,b]
