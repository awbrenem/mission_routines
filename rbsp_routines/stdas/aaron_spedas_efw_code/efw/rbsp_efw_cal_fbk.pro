...
;$LastChangedDate: 2020-01-03 16:13:58 -0600 (Fri, 03 Jan 2020) $
...
pro rbsp_efw_cal_fbk, probe=probe, trange=trange, datatype=datatype, pT=pT
	...
	;Get boom lengths and times for boom length determination
	if source7_13 eq '7' then get_data,'rbsp'+probe+'_efw_fbk_7_fbk_7',data=dd else $
		get_data,'rbsp'+probe+'_efw_fbk_13_fbk_13',data=dd
	times = dd.x

	bl = rbsp_efw_boom_deploy_history(times)
	...
	unit = strarr(2)
	offset = fltarr(2)
	boomlength = fltarr(n_elements(times),2)  ;2 for fb1 and fb2
...
;						gainf[*,ch]b = gainf[*,ch]
	...
	unit_conv = fltarr(n_elements(times),4)
	...
	if (channel[0] eq 'SCMU') or (channel[0] eq 'SCMV') or (channel[0] eq 'SCMW') then begin
		...
		unit_conv[*,0] = 1000./boomlength[*,0]
		unit_conv[*,1] = 1000./boomlength[*,0]
	endelse
	if (channel[1] eq 'SCMU') or (channel[1] eq 'SCMV') or (channel[1] eq 'SCMW') then begin
		...
		unit_conv[*,2] = 1000./boomlength[*,1]
		unit_conv[*,3] = 1000./boomlength[*,1]
	endelse
		...
		if tdexists(tvar[j],tr[0],tr[1]) then begin
			...
			if dlim.data_att.units eq 'ADC' then begin
				...
				if tst[0] ne 0 then begin
					...
					new_y = double(data.y)
					new_y =  (new_y - offsets[j]) * gains[j]
					if source7_13 eq '7'  then for f=0,6 do new_y[*,f] *= unit_conv[*,j]
					if source7_13 eq '13' then for f=0,12 do new_y[*,f] *= unit_conv[*,j]
