...
;		FREQ_BINS=freq_bins		<- array of frequency bins
;		/DB						<> if set, calculates power in dB: 10*log_10(2*time_step*fft^2)
;
;
;RETURNS:
...
function slide_spec,time,time_series,step,step_overlap,time_index=time_index,freq_bins=freq_bins,db=db,iwindow=iwindow,zero_pad=zero_pad
	...
	case iwindow of
		...
		1: window_array=(25./46.)-(21./46.)*cos(2.*!pi*lindgen(step_points)/step_points) ;Hammimg
		2: window_array=0.5-0.5*cos(2.*!pi*lindgen(step_points)/step_points) ;Hanning
		3: window_array=exp(-0.5*(3.*(lindgen(step_points)-step_points/2)/(step_points/2.))^2) ;Gaussian
		else:
	endcase
