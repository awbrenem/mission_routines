...
;;******
    ...
    store_data,plds[i]+'_alt',dtimed,alts6370
...
stop
    ...
    xgeo = alts6370*cos(!dtor*vals.lat)*cos(!dtor*vals.lon)
    ygeo = alts6370*cos(!dtor*vals.lat)*sin(!dtor*vals.lon)
    zgeo = alts6370*sin(!dtor*vals.lat)
...
;***TEST OUT MLAT/Lshell CALCULATIONs
;get_data,'G1_sm',data=pos_sm
;dr2a = sqrt(pos_sm.y[*,0]^2 + pos_sm.y[*,1]^2)
;dz2a = pos_sm.y[*,2]
;mlat_rad = atan(dz2a,dr2a)
;store_data,'G1_mlat',pos_sm.x,mlat_rad/!dtor
;
;Lshell_tst = alts6370/6370./(cos(!dtor*mlat_rad)^2)
;store_data,'G1_Lshelltst',pos_sm.x,Lshell_tst
;store_data,'G1_rad',pos_sm.x,alts6370/6370.


;; MLT of SC location (not foot point)
;get_data,'G1_gse',data=tmpgse
...
;store_data,'G1_MLTtst',data={x:tmpgse.x,y:mloctime}
;tinterpol_mxn,'G1_MLT',times,/overwrite,/quadratic;
;
;tplot,['G1_alt','G1_mlat','G1_Lshelltst','G1_rad','G1_MLTtst']



;get_data,'C1_sm',data=pos_sm
;dr2a = sqrt(pos_sm.y[*,0]^2 + pos_sm.y[*,1]^2)
;dz2a = pos_sm.y[*,2]
;mlat_rad = atan(dz2a,dr2a)
;store_data,'C1_mlat',pos_sm.x,mlat_rad/!dtor
;
;Lshell_tst = alts6370/6370./(cos(!dtor*mlat_rad)^2)
;store_data,'C1_Lshelltst',pos_sm.x,Lshell_tst
;store_data,'C1_rad',pos_sm.x,alts6370/6370.
;
;
;; MLT of SC location (not foot point)
...
;goo = where(mloctime ge 24.)
;if goo[0] ne -1 then mloctime[goo] = mloctime[goo] - 24
;store_data,'C1_MLTtst',data={x:tmpgse.x,y:mloctime}
;tinterpol_mxn,'C1_MLT',times,/overwrite,/quadratic;
;
;tplot,['C1_alt','C1_mlat','C1_Lshelltst','C1_rad','C1_MLTtst']



;;Test the traced footprint
;    cotrans,'C1_out_iono_foot_north_geo','C1_geiN',/geo2gei
;    cotrans,'C1_gei','C1_gseN',/gei2gse
;    cotrans,'C1_gse','C1_gsmN',/gse2gsm
;    cotrans,'C1_gsm','C1_smN',/gsm2sm
;
;
;get_data,'C1_smN',data=pos_sm
;dr2a = sqrt(pos_sm.y[*,0]^2 + pos_sm.y[*,1]^2)
;dz2a = pos_sm.y[*,2]
;mlat_rad = atan(dz2a,dr2a)
;store_data,'C1_mlatN',pos_sm.x,mlat_rad/!dtor
;

;Lshell_tst = alts6370/6370./(cos(!dtor*mlat_rad)^2)
;store_data,'C1_LshelltstN',pos_sm.x,Lshell_tst
;store_data,'C1_radN',pos_sm.x,alts6370/6370.
...
;See if what I've calculated for L, MLT, Mlat converts back to a sensible GEO coord.

;get_data,'C1_Lshelltst',data=lshell
;get_data,'C1_mlat',data=mlat
;get_data,'C1_MLTtst',data=mlt
;
;

;x = convert_lshell_mlt_mlat_to_geographic(lshell.y,mlat.y,mlt.y,lshell.x)
;
;store_data,'C1_geolon_tst',lshell.x,x.geolong
;store_data,'C1_geolat_tst',lshell.x,x.geolat
;store_data,'C1_radius_tst',lshell.x,x.radius
;
;store_data,'C1_lon_compare',data=['C1_geolon_tst','C1_geolon']
;store_data,'C1_lat_compare',data=['C1_geolat_tst','C1_geolat']
;options,'C1_lon_compare','colors',[0,250]
;options,'C1_lat_compare','colors',[0,250]
;
;tplot,['C1_lon_compare','C1_lat_compare']
...
;***********************************************
;COMPARE A FULL DAY OF ARASE ORBIT TO SEE HOW IT LOOK IN GEOGRAPHIC COORD
;***********************************************


;***********TEST
;GET FULL DAY OF FIREBIRD DATA IN GEO COOORD. MAP USING T89 AND COMPARE GEO OUTPUT 
;TO SSCWEB. 
;****************************************************************   
...
;    t1z = time_double('2020-08-17/00:00')
    t0z = time_double('2021-02-05/00:00')
    t1z = time_double('2021-02-06/00:00')
...
;    model = 'none'  ;IGRF
    ...
    if model eq 'none' then model = 'igrf'


    ;Compare T89 equatorial values to simple dipole calculation 
 ;   tplot,['G1!CL-shell-t89','G1!CL-shell-igrf','G1_Lshelltst']
 ;   tplot,['G1!Cequatorial-foot-MLT!Ct89','G1!Cequatorial-foot-MLT!Cigrf','G1_MLTtst']
 ;   ylim,['G1_out_eq_foot_gsm','G1_gsm'],-8d4,8d4
 ;   tplot,['G1_out_eq_foot_gsm','G1_gsm']
...
lmax = 15.
get_data,'C1!CL-shell-'+model,data=dd 
goo = where(dd.y ge lmax)
if goo[0] ne -1 then dd.y[goo] = !values.f_nan
store_data,'C1!CL-shell-'+model,data=dd 

get_data,'C1!Cnorth-foot-MLT!C'+model,data=dd
if goo[0] ne -1 then dd.y[goo] = !values.f_nan
store_data,'C1!Cnorth-foot-MLT!C'+model,data=dd 
get_data,'C1!Csouth-foot-MLT!C'+model,data=dd
if goo[0] ne -1 then dd.y[goo] = !values.f_nan
store_data,'C1!Csouth-foot-MLT!C'+model,data=dd 
...
tplot,['C1!CL-shell-'+model,'C1_MLT','C1!Cnorth-foot-MLT!C'+model,'C1!Csouth-foot-MLT!C'+model]
...
dif_data,'C1!CL-shell-'+model,'G1!CL-shell-'+model,newname='C1G1_Ldiff'
dif_data,'C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model,newname='C1G1N_MLTdiff'
dif_data,'C1!Csouth-foot-MLT!C'+model,'G1!Csouth-foot-MLT!C'+model,newname='C1G1S_MLTdiff'
dif_data,'C1_MLT','G1!Cnorth-foot-MLT!C'+model,newname='C1G1NE_MLTdiff'
dif_data,'C1_MLT','G1!Csouth-foot-MLT!C'+model,newname='C1G1SE_MLTdiff'
...
store_data,'C1G1_Lcomb',data=['C1!CL-shell-'+model,'G1!CL-shell-'+model]
...
;tplot,['C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model,'C1G1N_MLTdiff']

;TESTING
;    options,['C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model,'C1G1N_MLTdiff','C1G1N_MLTdiff_tmp'],'psym',-4
;    store_data,'combtmp',data=['C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model]
;    options,'combtmp','colors',[0,250]
;tplot,['C1!CL-shell-'+model,'combtmp','C1G1N_MLTdiff_tmp','C1G1N_MLTdiff']
...
;    options,['C1_MLT','G1!Cnorth-foot-MLT!C'+model,'C1G1NE_MLTdiff','C1G1NE_MLTdiff_tmp'],'psym',-4
;    store_data,'combtmp',data=['C1_MLT','G1!Cnorth-foot-MLT!C'+model]
;    options,'combtmp','colors',[0,250]
...
;-------------------------------------------------------------------------------
    ...
    options,['C1_geolat','C1G1N_absolute_conjunction','C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model,'C1G1N_MLTdiff_abs','C1G1_Ldiff_abs_comb'],'psym',-4
    tplot,['C1_geolat','C1G1N_absolute_conjunction','C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model,'C1G1N_MLTdiff_abs','C1G1_Ldiff_abs_comb']
    ...
    options,['C1_geolat','C1G1S_absolute_conjunction','C1!Csouth-foot-MLT!C'+model,'G1!Csouth-foot-MLT!C'+model,'C1G1S_MLTdiff_abs','C1G1_Ldiff_abs_comb'],'psym',-4
    tplot,['C1_geolat','C1G1S_absolute_conjunction','C1!Csouth-foot-MLT!C'+model,'G1!Csouth-foot-MLT!C'+model,'C1G1S_MLTdiff_abs','C1G1_Ldiff_abs_comb']
    ...
    tplot,['C1_geolat','C1G1NE_absolute_conjunction','C1_MLT','G1!Cnorth-foot-MLT!C'+model,'C1G1NE_MLTdiff_abs','C1G1_Ldiff_abs_comb']
    ...
    tplot,['C1_geolat','C1G1SE_absolute_conjunction','C1_MLT','G1!Csouth-foot-MLT!C'+model,'C1G1SE_MLTdiff_abs','C1G1_Ldiff_abs_comb']
...
stop
    ...
    store_data,'C1G1N_MLT_comb',data=['C1!Cnorth-foot-MLT!C'+model,'G1!Cnorth-foot-MLT!C'+model]
    ...
    store_data,'C1G1S_MLT_comb',data=['C1!Csouth-foot-MLT!C'+model,'G1!Csouth-foot-MLT!C'+model]
    ...
    store_data,'C1G1NE_MLT_comb',data=['C1_MLT','G1!Cnorth-foot-MLT!C'+model]
    ...
    store_data,'C1G1SE_MLT_comb',data=['C1_MLT','G1!Csouth-foot-MLT!C'+model]
...
;    map_set,/mercator,0,0,/grid,label=1,title='Title'
;    map_set,/azimuthal,0,0,/grid,label=1,title='Title'
;    map_set,/lambert,0,0,/grid,label=1,title='Title'
    map_set,/mollweide,0,0,/grid,label=1,title='Title'
    map_continents

;    ;Boulder
;    t0z2 = time_double('2020-08-16/00:14')
;    t1z2 = time_double('2020-08-16/00:17')
