...
kr = [-5,5]
#tz = 887
#nsec = 0.04   #set to zero to select (slice) out a single spectral time
tz = 881
nsec = 1   #set to zero to select (slice) out a single spectral time
yr = [0,10000]
...
receiver_spacing = 2.27 #meters -- effective length of spaced receiver (=3*cos(45))
...
tchunk = 0.02  #delta-time (sec) for each time chunk to divide up the spectra into
#nchunks = int(np.ceil((wfAx.size/fs)/tchunk)) #number of chunks in ENTIRE timerange

cohx2, phasex2, tchunks2, freqs2 = correlation_analysis.cross_spectral_density_spectrogram(wfAx,wfBx,tdatx,fs,tchunk,coh_min=cohmin,nperseg=64)
cohy2, phasey2, tchunks2, freqs2 = correlation_analysis.cross_spectral_density_spectrogram(wfAy,wfBy,tdaty,fs,tchunk,coh_min=cohmin,nperseg=64)
...
fig,axs = plt.subplots(5,2, figsize=(8,8))  #,gridspec_kw={'height_ratios':[1,1,1,1,1,1,1,1,1]})
...
#ps.plot_spectrogram(tspecxz,fspecx,np.abs(powarry2),vr=vr,xr=xrspec,yr=yr,yscale='linear',ax=axs[1,0],xlabel='time(sec)',ylabel="power spectrum y'\nfreq(Hz)")
...
for i in range(4): axs[i,0].axvline(tz, linestyle='--')
for i in range(4): axs[i,0].axvline(tz+nsec, linestyle='--')
...
wlim = (2 * receiver_spacing)
for i in range(2): axs[i,1].axvline(klim,linestyle="--")
for i in range(2): axs[i,1].axvline(-klim,linestyle="--")
...
axs[2,1].axvline(wlim,linestyle="--")
...
ps.plot_spectrogram(kvalsx,fvalsx,pmaxvalsx,vr=[0,1],xr=krplot,yr=yr,zscale='linear',yscale=ys,ax=axs[0],minzval=0,maxzval=1,xlabel='k(rad/m)',ylabel="kx'\n"+xptitle+'\nf(Hz)',cmap='Greys',title=titlegoo)
ps.plot_spectrogram(kvalsx,fvalsx,fkpowspecx,vr=vr,xr=krplot,yr=yr,yscale=ys,ax=axs[0],minzval=-120,maxzval=10,xlabel='k(rad/m)',ylabel="kx'\n"+xptitle+'\nf(Hz)',alpha=0.5)
ps.plot_spectrogram(kvalsy,fvalsy,pmaxvalsy,vr=[0,1],xr=krplot,yr=yr,zscale='linear',yscale=ys,ax=axs[1],minzval=0,maxzval=1,xlabel='k(rad/m)',ylabel="ky'\n"+yptitle+'\nf(Hz)',cmap='Greys')
ps.plot_spectrogram(kvalsy,fvalsy,fkpowspecy,vr=vr,xr=krplot,yr=yr,yscale=ys,ax=axs[1],minzval=-120,maxzval=10,xlabel='k(rad/m)',ylabel="ky'\n"+yptitle+'\nf(Hz)',alpha=0.5)

axs[0].axvline(klim, linestyle='--')
axs[0].axvline(-klim, linestyle='--')
axs[1].axvline(klim, linestyle='--')
axs[1].axvline(-klim, linestyle='--')
...
axs[2].plot(freqs2,wl2,'*',color='black')
axs[2].set_xlim(yr)
axs[2].set_ylim(0,20)
