...
tmpv0 = eea_data["eeas_hires_eflux_gsfc"][:,:,0]  #0 deg PA bin (downgoing)
tmpv180 = eea_data["eeas_hires_eflux_gsfc"][:,:,19]  #180 deg PA bin (upgoing)
...
plt.pcolormesh(tmpt[goodt], tmpe, np.transpose(tmpv0[goodt,:]), shading="gouraud")
plt.yscale('log')
plt.ylim(1, 30000)
plt.title("EAAs 0 deg electrons on 35.040 (low flyer)")
plt.ylabel("Energy [eV]")
plt.xlabel("MET (sec)")


#---------------------------------------------------------------------
#Verify cusp entry/exit 
...
pcm2 = axs[1].pcolormesh(tmpt, tmpe, np.transpose(tmpv0), shading="gouraud")
